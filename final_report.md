...
Support for such external query execution (i.e., execution beyond available memory) already exists in DataFusion, but it remains an active area of community work. The long-term goal is to make spilling more robust and transparent, so queries under memory pressure degrade gracefully instead of failing outright. This project contributes to that effort by:
